...
@Component({
    selector: 'app-hodl',
    templateUrl: './hodl.component.html',
    styleUrls: ['./hodl.component.css']
})
export class HodlComponent extends SuperComponent implements OnInit {
...
tx;tx2;
    sendOffer(type) {
        ...
        if (valid) {
            this._globalService.loading(true);
            this.service.sendbuy(offer).subscribe(response => {
                this._globalService.loading(false);
    ...
    sendOffer2(type) {
        let buying = type == 'sell' ? 'XLM' : 'UNSK';
        let selling = type == 'sell' ? 'UNSK' : 'XLM';
        let amount = type == 'sell' ? this.sellAmount.value : this.buyAmount.value;
        let price = type == 'sell' ? this.sellPrice.value : this.buyPrice.value;
        let offer = {
            selling: selling,
            buying: buying,
            amount: amount,
        ...
        let valid = type == 'sell' ? (this.sellForm.valid) : (this.buyForm.valid);
        if (valid) {
            this._globalService.loading(true);
            this.service.sendsell(offer).subscribe(response => {
                this._globalService.loading(false);
                this._toastr.success('Offer has been bid successfully');
                this.sellForm.reset();
                this.tx2=response.tx;
                this.sellForm.reset();
            }, error => {
                const errorData = error.error;
                this._globalService.loading(false);
